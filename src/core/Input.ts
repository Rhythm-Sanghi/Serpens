...
export class InputHandler {
  ...
  public setCellSize(size: number) { this.minSwipeDistance = Math.max(20, size * 0.7); }
  ...
  public getNextDirection(stateDirection: Direction): Direction {
    if (this.directionQueue.length === 0) return stateDirection;
    const next = this.directionQueue.shift()!;
    const isOpposite = 
      (next === Direction.UP && stateDirection === Direction.DOWN) ||
      (next === Direction.DOWN && stateDirection === Direction.UP) ||
      (next === Direction.LEFT && stateDirection === Direction.RIGHT) ||
      (next === Direction.RIGHT && stateDirection === Direction.LEFT);

    if (!isOpposite) return next;
